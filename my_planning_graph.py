...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if all(node in self.s_levels[level]
                   for node in action_node.prenodes):
                parent_nodes = [node for node in self.s_levels[level]
                                if node in action_node.prenodes]

                for i, ni in enumerate(parent_nodes):
                    if ni.mutex.intersection(parent_nodes[i:]):
                        break
                else:
    ...
    def add_literal_level(self, level):
        ...
        for action_node in self.a_levels[level - 1]:
            for node in action_node.effnodes:
                if node not in s_nodes_map:
                    s_nodes_map[node] = node

                s_node = s_nodes_map[node]
                s_node.parents.add(action_node)
                action_node.children.add(s_node)

        self.s_levels.append(set(s_nodes_map.values()))
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        pre1 = set(node_a1.prenodes)
        pre2 = set(node_a2.prenodes)

        for effnode in node_a2.effnodes:
            if PgNode_s(effnode.symbol, not effnode.is_pos) in pre1:
                return True

        for effnode in node_a1.effnodes:
            if PgNode_s(effnode.symbol, not effnode.is_pos) in pre2:
                return True
    ...
    def h_levelsum(self) -> int:
        ...
        '''
        level_sum = 0
        remaining_clauses = set(self.problem.goal)
        for i, states in enumerate(self.s_levels):
            for s in states:
                if s.literal in remaining_clauses:
                    level_sum += i
                    remaining_clauses.remove(s.literal)
